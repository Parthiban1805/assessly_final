...
import { GOOGLE_CLIENT_ID } from "./config/constants"; // Import GOOGLE_CLIENT_ID from constants
import { BreadcrumbProvider } from "./contexts/BreadcrumbContext"; // Breadcrumb context provider
import AppLayout from './layouts/AppLayout'; // Main application layout component
...
import NotFoundPage from './routes/NotFoundPage'; // 404 Not Found Page
...
import StudentResultPage from "./routes/Student/Assessment/ResultPage"; // Renamed to avoid conflict with TeacherResultPage
...
import TeacherResultPage from "./routes/Teacher/ResultPage"; // This is the teacher's version
import TeacherDashboard from "./routes/Teacher/TeacherDashboard";


/**
 * RoleBasedRedirect component determines the appropriate dashboard to redirect
 * an authenticated user to based on their assigned role.
 * @returns {JSX.Element} A Navigate component to the correct dashboard.
 */
const RoleBasedRedirect = () => {
    ...
    switch (user.role) {
        ...
        default: return <Navigate to="/login" replace />; // Fallback to login for unknown roles.
    }
};

/**
 * Main App component. Sets up routing, authentication, and theme contexts.
 * Configures Google OAuth Provider.
 * Defines public, protected, and layout-wrapped routes.
 * @returns {JSX.Element} The root application component.
 */
const App = () => {
    return (
        // GoogleOAuthProvider wraps the entire app to enable Google login.
        <GoogleOAuthProvider clientId={GOOGLE_CLIENT_ID}>
            {/* BrowserRouter enables client-side routing. */}
            <BrowserRouter>
                {/* AuthProvider provides authentication state globally. */}
                <AuthProvider>
                    {/* ThemeProvider provides theme (light/dark) state globally. */}
                    <ThemeProvider>
                        {/* BreadcrumbProvider manages breadcrumb navigation paths. */}
                        <BreadcrumbProvider >
                            <Routes>
                                {/* ====================================================== */}
                                ...
                                {/* which do not include the main AppLayout (sidebar/breadcrumbs). */}
                                {/* =================================================================== */}
                                ...
                                {/* All standard dashboard pages and content management sections go here. */}
                                {/* They use `AppLayout` which provides the common UI structure. */}
                                {/* ==================================================================== */}
                                <Route
                                    path="/"
                                    element={
                                        ...
                                        <ProtectedRoute>
                                            {/* AppLayout provides the common layout (sidebar, top bar, content area). */}
                                            <AppLayout />
                                        </ProtectedRoute>
                                    }
                                >
                                    {/* Index route redirects based on user role to their respective dashboard. */}
                                    ...
                                    <Route path="results/:assessmentId" element={<ProtectedRoute allowedRoles={['student']}><StudentResultPage /></ProtectedRoute>} />
                                    ...
                                    <Route path="teacher-dashboard/students/:studentId/:assessmentId" element={<TeacherResultPage />} /> {/* Teacher's view of a student's specific result */}
                                ...
                                {/* A final catch-all route for any path not matched above, leading to 404 page. */}
